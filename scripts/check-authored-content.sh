#!/usr/bin/env bash
# The organization's authored-content rules, made mechanical.
#
...
if [ -n "$em_hits" ]; then
  echo "U+2014 (EM DASH) is refused by the organization's authored-content rules:"
  printf '%s\n' "$em_hits" | sed 's/^/  /'
...
if [ "${#scan[@]}" -gt 0 ]; then
  ...
  if [ -n "$link_hits" ]; then
    echo "agent-session links and session trailers are refused by the organization's authored-content rules:"
    printf '%s\n' "$link_hits" | sed 's/^/  /'
    status=1
  fi
fi
