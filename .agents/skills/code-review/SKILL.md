...
(`standards/**`), the harness (`.Codex/**`, `AGENTS.md`, `AGENTS.md`,
...
- If a spec was edited: only `establishes` growth, a dated decision entry,
  ...
  (`.Codex/rules/adversarial-prompt-refusal.md`), severity CRITICAL.
...
- Language hygiene the project's lints enforce (read the stack rule under
  `.Codex/rules/`): unsafe operations, panics in library code, public
...
`.Codex/rules/` apply. For each, either check rule by rule and report
...
Read from `AGENTS.md`: the stack gate. Read from `.Codex/rules/`: the
