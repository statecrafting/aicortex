---
name: refactor-Codex-md
description: "Tighten AGENTS.md by extracting context-specific guidance into docs and path-scoped rules under .Codex/rules, keeping the harness spec coupled and the index fresh."
allowed-tools: Bash, Read, Write, Edit, Glob, Grep
argument-hint: "[path to AGENTS.md, default ./AGENTS.md]"
---

# Refactor AGENTS.md

Reduce the size of `AGENTS.md` while preserving guidance, by moving
context-specific sections into `docs/` and loading them through
path-scoped rules. In a spec-spine repository `AGENTS.md`,
`.Codex/rules/`, and `AGENTS.md` are usually hashed inputs of the
...
1. **Read and analyze** the current `AGENTS.md` in full. Compare it with
   `AGENTS.md`: anything duplicated between them belongs in `AGENTS.md`
   only (it is the cross-agent authority; `AGENTS.md` carries only what
   Codex needs beyond it).
...
3. **For each candidate** recommend: the doc name under `docs/`, its
   ...
   reference to keep in `AGENTS.md`.

4. **Create the files** in this order: extract to `docs/<name>.md`;
   create `.Codex/rules/<name>.md` with `paths:` frontmatter and a short
   reminder pointing at the doc; replace the extracted section in
   `AGENTS.md` with the reference; update any documentation table.
...
- Extract only context-specific guidance; keep universal rules in
  `AGENTS.md`.
- Preserve critical information in `AGENTS.md`: the invariants, the
  commands, the architecture table, the governance mechanics, house
  style.
- Choose meaningful path patterns; a rule that loads everywhere is a `AGENTS.md` section
  in disguise.
