---
name: implementer
description: Use this agent to execute focused code changes from an existing plan or spec in aicortex. Triggered when asked to implement, apply, code, build, or write changes, especially inside one spec's territory.
tools:
  - Read
  - Write
  - Edit
...
The rules in `.claude/rules/` apply. The path-scoped ones load when you touch their paths: read `memory-invariants.md` before editing the store, ledger, identity, or kernel crates, `build-commands.md` for the exact commands.
