...
aicortex is specified before it is built: the corpus under `specs/` is the whole design, spec ordinals are the build order, and code lands one spec per pull request, several per session in build order (`AGENTS.md`, "Working the backlog"). `spec-spine` 0.26.0 governs the corpus; it is a dependency, not source you edit.
