...
kind: kernel                   # constitutional-bootstrap | thesis | governance | kernel | feature | constraint | tooling
domain: memory                 # governance | chassis | memory | retrieval | protocol | clients | ingest | ops
created: "YYYY-MM-DD"
authors: ["Bartek Kus"]
...
wave: 1                        # build-order wave, 1..4 (spec 002 §5)
depends_on:
...
# establishes:
#   - "crates/aicortex-x/Cargo.toml"                      # a crate-founding spec claims its manifest
#   - "crates/aicortex-x/src/lib.rs"
#   - "crates/aicortex-x/src/thing.rs"                    # every source file, explicitly
#   - "crates/aicortex-x/tests/thing.rs"
#   - "crates/aicortex-x/testdata/thing/"                 # fixtures as a subtree
# extends:
#   - { spec: "NNN-founder", unit: "crates/aicortex-x/src/lib.rs", nature: additive }           # re-exports
#   - { spec: "NNN-founder", unit: "crates/aicortex-x/Cargo.toml", nature: additive }            # new deps
#   - { spec: "010-chassis-adoption-and-workspace", unit: { kind: section, file: "Cargo.toml", anchor: "workspace.dependencies" }, nature: additive }
# constrains:
#   - { flavor: invariant-freeze, unit: "crates/aicortex-store/src/txn.rs", note: "one txn is the atomic unit" }
# references:
...
constitutional principle it serves. One or two paragraphs. Cite an observed
failure of the prior system by `openbrain://topic`, and a chassis contract
this spec relies on by `rahi://NNN`.
...
- **AC-1.** `cargo test -p aicortex-x --locked` passes.
...
```verify:cli
cargo test -p aicortex-x --locked
```
